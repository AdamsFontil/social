...
import { fetchSpecificProfile } from "../api/fetchSpecificProfile"
 ...
 const searchParams = useSearchParams();
  ...
  useEffect(() => {
    ...
    const fetchUserByUsername = async () => {
      try {
        const userData = await fetchSpecificProfile(username);
        setUser(userData);
        // setUser(null)  //to test !user
        console.log('api call result----', userData)
  ...
  return (
    <div>
      ...
      <section className="stats flex flex-col gap-2 text-gray-500 text-lg p-4">
        <div className="flex text-2xl flex-col">
          <h4 className="text-white">{user?.display_name}</h4>
